"""
実験3（Tiny Shakespeareを用いた文章生成問題，Decoder-only Transformer）Stage A
（安定性探索）の学習ループを定義し，実験を実行するスクリプト．

`.orders/order_027.md` に対応する．実験2（CIFAR-10）が正規化層なしのモデルでいきなり本比較
を実施した結果，9条件中7条件でNFG SVRG・ASAI SVRGが発散するという想定外の結果に直面し，
その後3段階（ex0021〜ex0023）の追加実験を要したという教訓を踏まえ，本実験3は最初から
段階的に実施する．Stage Aでは，NFG SVRG・ASAI SVRGの2手法に絞った小規模なグリッド探索を
短いエポック数で実施し，発散・崩壊が生じない条件を特定する（`.orders/order_027.md` 6節）．

## 比較手法（`.orders/order_027.md` 5節）

`programs/ex0022_cifar10_alexnet_groupnorm/train.py`・`programs/
ex0023_cifar10_alexnet_groupnorm_longrun/train.py` と同一の実装（`SVRG`／`NFGSVRG`，次の
スナップショット点を内部ループのパラメータ列からランダムに選ぶ，ASAI SVRG論文Algorithm 4の
理論解析に整合する版）をそのまま用いる．
...
def run_variance_reduced(
    ...
    概要: NFG SVRG・ASAI SVRGによる学習を実行し，各エポック終了時の評価指標を `logger` に
        記録する．NFG SVRG・ASAI SVRGは評価指標（近似誤差）算出のためだけに真のフル勾配を
...
def main():
    """
    概要: 実験3 Stage Aの全条件（2手法 × 3バッチサイズ × 2学習率 × 3Seed = 36条件）を
        マルチプロセスで並列に学習する．
    引数: なし
    戻り値: なし
    """
    print(
        ...
        f"lambda = {REG_LAMBDA}, epochs = {STAGE_A_EPOCHS}, vocab_size = {_VOCAB_SIZE}"
    )
