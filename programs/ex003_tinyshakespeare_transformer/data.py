"""
実験3（Tiny Shakespeareを用いた文章生成問題，Decoder-only Transformer）のデータセットの
定義に関するモジュール．

`.orders/order_027.md` 3節の指示に基づき，文字レベルの言語モデリングとして実装する．文字→ID
変換のみの決定論的なトークナイズを行い，コーパス全体を固定長 $ T $ の非重複のチャンクに
分割する．各チャンクを1サンプル $ n $ として扱い，有限和最小化の枠組み
（$ f(w)=\\frac1N\\sum_n f_n(w) $）に対応させるため，チャンクは学習の全過程を通じて固定する
（エポックごとに切り出し位置を変えるスライディングウィンドウ等は用いない）．

コーパスの前方90%を学習用，後方10%を検証用とする．学習・検証それぞれの範囲内で独立に
非重複チャンクへ分割するため，学習用チャンクと検証用チャンクの間で文字が重複することはない．
"""
