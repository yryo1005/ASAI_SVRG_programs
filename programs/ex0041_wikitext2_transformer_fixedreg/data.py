...
Transformer）のデータセットの定義に関するモジュール．

`.orders/order_031.md` 2節の指示に基づき，`programs/ex004_wikitext2_transformer/data.py`
と完全に同一のデータセット・前処理（トークナイズ，語彙構築，系列分割，学習/検証分割）を
用いる．WikiText-2は，PyTorch公式のword language modelサンプル
...
`valid.txt`を検証用として用いる（`test.txt`は本実験でも使用せず，将来の最終評価用に温存する）．

コーパス全体（改行を含む全トークン）を単純に連結した1本のトークン列とみなし，固定長 $ T $ の
非重複チャンクに分割する．各チャンクを1サンプル $ n $ として扱う点は実験3・実験4と同一である．
"""
...
SEQUENCE_LENGTH = 64  # 1サンプルあたりの入力トークン数 T（実験4と同一）
UNK_TOKEN = "<unk>"
