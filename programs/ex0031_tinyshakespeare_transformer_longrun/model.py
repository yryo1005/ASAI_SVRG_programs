"""
実験3 Stage C（長期学習によるASAI SVRGの効率性優位性の持続性検証）のモデルの定義に関する
モジュール．

`.orders/order_029.md` の指示に基づき，`programs/ex003_tinyshakespeare_transformer/model.py`
をそのまま複製したものである．小規模なDecoder-only Transformer（`DecoderOnlyTransformer`）
を定義する．SVRG系手法の理論的前提（同一の $ n $・同一のパラメータ $ z_s $ に対する勾配評価が
常に同じ値になること）を満たすため，Dropout・BatchNormalizationは一切使用しない．正規化には
LayerNorm（各トークンごとに特徴次元方向へ正規化するため，バッチ内の他サンプルに依存しない）を
Pre-LN構成で用いる．位置エンコーディングは学習可能な埋め込み（`nn.Embedding`，モデル
パラメータの一部）とする．Causalマスクは決定論的な操作であるため問題ない．
"""
