"""
実験4（WikiText-2を用いた単語レベル言語モデリング）のモデルの定義に関するモジュール．

`.orders/order_030.md` 3節・5節の指示に基づき，`programs/ex003_tinyshakespeare_transformer/
model.py` と同一のアーキテクチャ（小規模なDecoder-only Transformer，`DecoderOnlyTransformer`）
を用いる．SVRG系手法の理論的前提（同一の $ n $・同一のパラメータ $ z_s $ に対する勾配評価が
常に同じ値になること）を満たすため，Dropout・BatchNormalizationは一切使用しない．正規化には
LayerNorm（各トークンごとに特徴次元方向へ正規化するため，バッチ内の他サンプルに依存しない）を
Pre-LN構成で用いる．位置エンコーディングは学習可能な埋め込み（`nn.Embedding`，モデルパラメータ
の一部）とする．Causalマスクは決定論的な操作であるため問題ない．

語彙サイズが実験3（65）から本実験（約33,000）へ大幅に増加するため，`token_embedding`・`head`
（出力射影層）のパラメータ数が全体の9割程度を占める（`.reports/report_030.md` 5節の見積もり
参照）．Transformer本体（`D_MODEL`・`N_HEAD`・`N_LAYER`・`D_FF`）は実験3と同一の規模を維持する．
"""
...
class DecoderOnlyTransformer(nn.Module):
    """
    概要: 小規模なDecoder-only Transformer（単語レベル次単語予測）．
    """
