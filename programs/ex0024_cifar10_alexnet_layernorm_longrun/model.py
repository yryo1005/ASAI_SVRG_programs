...
`.orders/order_026.md` 2節の指示に基づき，`programs/ex0022_cifar10_alexnet_groupnorm/model.py`
（さらにその複製元である`programs/ex0021_cifar10_alexnet_norm/model.py`）をそのまま複製した
ものである．本実験ではGroupNormのみを使用するが，比較のためのコード自体は削らず，
`AlexNetCIFARNorm` が持つ正規化層4パターン（なし，LayerNorm，GroupNorm，SpectralNorm）の
選択肢をそのまま保持する．
