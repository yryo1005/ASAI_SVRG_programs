"""
実験ex0022（GroupNorm下での4手法比較，分散削減の効率性検証）のデータセットの定義に関する
モジュール．

`.orders/order_025.md` の指示（「データセット・前処理・目的関数は実験2と同一」）に基づき，
`programs/ex002_cifar10_alexnet/data.py`／`programs/ex0021_cifar10_alexnet_norm/data.py` と
同一の前処理（公式の学習・検証分割，チャネルごとの標準化，データ拡張なし）を用いる．
"""
