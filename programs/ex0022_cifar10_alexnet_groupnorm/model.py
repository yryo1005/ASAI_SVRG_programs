"""
実験ex0022（GroupNorm下での4手法比較，分散削減の効率性検証）のモデルの定義に関するモジュール．

`.orders/order_025.md` 2節の指示に基づき，`programs/ex0021_cifar10_alexnet_norm/model.py` を
そのまま複製したものである．本実験ではGroupNormのみを使用するが，比較のためのコード自体は
削らず，`AlexNetCIFARNorm` が持つ正規化層4パターン（なし，LayerNorm，GroupNorm，
SpectralNorm）の選択肢をそのまま保持する．
