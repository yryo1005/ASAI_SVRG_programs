"""
実験0（a9aデータセットを用いた二値分類問題）のデータセットの定義に関するモジュール．

a9a（LIBSVM Data，UCI Adult所得予測データセットの二値分類向け前処理版）を読み込み，
NFG SVRG原論文（`references/No_Full_Grad_SVRG.pdf`）付録A.1（LEAST SQUARES REGRESSION）の
式(8)に対応するデータセットを構築する．a9aは，1レコードあたり123次元のカテゴリ変数
（One-Hotエンコード済み，各成分は0/1の二値）と，二値ラベル（年収が5万ドルを超えるか否か，
元データでは-1/+1）から構成される．

原論文の式(8)は特徴量ベクトル A_i をそのまま線形結合 z_i = A_i・x に用いる形で定式化されており，
...
def download_raw_data():
    ...
    response = requests.get(RAW_DATA_URL, timeout=120)
...
def build_features_and_labels() -> tuple[np.ndarray, np.ndarray]:
    ...
    概要: 生データ（LIBSVM形式）から，特徴量行列とラベルベクトルを構築する．
        原論文の式(8)の非線形最小二乗損失は y_i を h_i = 1/(1+exp(-z_i)) ∈ (0, 1) と直接
        比較するため，元データのラベル（-1: 年収5万ドル以下，+1: 年収5万ドル超）を
        {0, 1} へ写像する．
    引数: なし
    戻り値:
...
def load_dataloader(seed: int = 0, batch_size: int = 1) -> tuple[DataLoader, DataLoader]:
    ...
    概要: 学習用および検証用のデータローダーをインスタンス化するための関数．
        全データを9:1で学習用・検証用に分割する．原論文の式(8)の定式化（z_i = A_i・x）に
        忠実に，特徴量は標準化等を行わず0/1の値をそのまま用いる．
    引数:
        seed (int) = 0．データ分割・シャッフルの再現性を担保する乱数シード．
        batch_size (int) = 1．データローダーのミニバッチサイズ（実験0ではオンライン学習の
            設定に合わせ1）．
    戻り値:
