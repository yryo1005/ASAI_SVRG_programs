...
`.orders/order_010.md` の指示に基づくEx005（σを外した純粋なCIFAR-10分類問題）に対応する
データセットを構築する．CIFAR-10は32×32ピクセルのカラー画像からなる10クラスの画像分類
データセットであり，訓練データ数 N = 50000，テストデータ数 10000 が公式に定義されている．
Ex003・Ex004の`data.py`と，前処理（画素値の標準化）を含め同一である．
"""
...
RAW_DATA_DIR = os.path.join(_PROJECT_ROOT, "datasets", "ex005_cifar10_resnet_classification", "raw")
...
def load_dataloader(seed: int = 0, batch_size: int = 128) -> tuple[DataLoader, DataLoader]:
    ...
    # num_workers>0を指定した子プロセスを追加で生成することはできないため，num_workers=0
    # （メインプロセス内での読み込み）とする．
