...
分類，min-max敵対的ロバスト性の定式化）を再現する数値実験スクリプト（Ex004）．

Ex003（`programs/ex003_cifar10_resnet_minmax/`，`.reports/report_007.md`）は，原論文の
「ワーカー数M=5による分散環境の模擬」を単一プロセスでのミニバッチ学習として簡略化していた．
`.orders/order_009.md` の指示に基づき，Ex004ではこの簡略化を解消し，次の3点を修正する
（ミニバッチサイズ128は維持する）．

1. **M=5ワーカーによる分散環境の模倣とBatch Normalization挙動の修正**：グローバルミニバッチ
   （サイズ128）を，`M_WORKERS`（=5）個のローカルサブバッチ（サイズ約25〜26）に分割し，
   各サブバッチを独立にforwardすることで，各ワーカーが自身のローカルミニバッチに対して
   Batch Normalizationを適用する挙動を模倣する（`backward_minmax_objective_distributed`）．
   データ項（交差エントロピー損失）の勾配は各サブバッチのサンプル数で重み付け平均し，
   正則化項の勾配はグローバルミニバッチ全体に対して1回だけ加える．
2. **フル勾配計算時のBatch Normalization統計量の固定**：`compute_full_gradient_and_metrics`
   は，全データを走査する間，Batch Normalizationの移動平均統計量（running_mean，
   running_var）が走査の途中で更新されると，走査の前半と後半で異なる関数を評価してしまい，
   スナップショット勾配としての数学的整合性が崩れる．`model.eval()`（移動平均統計量を固定し，
   ミニバッチ統計量を用いない評価モード）を用いることで，全パラメータに対する勾配計算
   （`.grad`）を有効に保ちつつ，統計量の意図しない更新を防ぐ．
3. **sigmaの正則化勾配のスケール**：上記1の実装（サブバッチ分割・重み付け平均）において，
   正則化項 -(lambda2/2)||sigma||^2 の勾配をグローバルミニバッチ全体に対して明示的に1回だけ
   加えることで（サブバッチごとに重複して加算しない），データ項の勾配（reduction="mean"に
   相当する重み付け平均）とのスケール整合性を保証する．
...
- ハイパーパラメータ：学習率 gamma = 0.01（w, sigma共通），lambda1 = lambda2 = 0.0005（原論文
  7節）．ミニバッチサイズは原論文が明記していないため，Ex001〜Ex003と同様の128を用いる．
...
EXPERIMENT_NAME = "ex004_cifar10_resnet_minmax"
...
# 実験時間の制約（1エポックあたりSVRG系手法で約80秒，4手法 x 5Seed，かつM_WORKERSによる
# サブバッチ分割で追加の計算コストが発生する）を考慮し30エポックとした．
EPOCHS = 30
...
# 保つ必要がある）とは独立に，計算効率のためこの値を用いる．なお，フル勾配計算は
# `compute_full_gradient_and_metrics` の修正（Batch Normalization統計量の固定）により
# `model.eval()` で実行するため，ここでのミニバッチ分割はBatch Normalizationの挙動に影響しない．
FULL_GRADIENT_BATCH_SIZE = 512
# 原論文が想定するワーカー数M（分散環境の模擬）．グローバルミニバッチ（BATCH_SIZE）を
# M_WORKERS個のサブバッチに分割し，各サブバッチを独立にforwardすることで，各ワーカーが
# 自身のローカルミニバッチに対してBatch Normalizationを適用する挙動を模倣する．
M_WORKERS = 5
...
def backward_minmax_objective_distributed(
    ...
    概要: グローバルミニバッチを `num_workers` 個のサブバッチ（ワーカー）に分割し，各サブバッチを
        ...
        値）を`.grad`に累積する．正則化項 (lambda1/2)||w||^2 - (lambda2/2)||sigma||^2 の勾配は
        サブバッチ数に依らずグローバルミニバッチ全体に対して1回だけ加える（`backward_minmax_
        objective`とスケールを一致させるため）．最後にsigma成分の勾配の符号を反転させ，
        F(z) = (grad_w L; -grad_sigma L) を得る．
    引数:
        model (torch.nn.Module)．`MinMaxResNet18` のインスタンス．
    ...
    w_norm_sq, sigma_norm_sq = compute_squared_norms(model)
    regularization = (LAMBDA1 / 2) * w_norm_sq - (LAMBDA2 / 2) * sigma_norm_sq
...
def iteration(model, inputs, teacher_signals, optimizer=None, snapshot_model=None) -> dict:
    ...
    概要: 1つのミニバッチのデータを学習/検証する関数．
        ...
        模倣する `backward_minmax_objective_distributed` を用いる．
    ...
    if optimizer is None:
        ...
        outputs = backward_minmax_objective_distributed(model, inputs, teacher_signals)
        loss = loss_func(outputs, teacher_signals)

        if snapshot_model is not None:
            backward_minmax_objective_distributed(snapshot_model, inputs, teacher_signals)
...
def compute_full_gradient_and_metrics(model, dataloader, device):
    ...
    概要: データローダー全体に対する min-max 目的関数のフル勾配 F(z)，および同じ1回の
        走査で得られる誤差・分類精度の平均値をまとめて計算する．
        全データを走査する間，Batch Normalization層が学習モード（`model.train()`）の
        ままだと，移動平均統計量（running_mean，running_var）が走査の途中で更新され続け，
        走査の前半と後半で異なる関数を評価してしまい，スナップショット勾配としての数学的
        整合性が崩れる．そこで `model.eval()`（移動平均統計量を固定し，ミニバッチ統計量を
        用いないモード）に設定した上でforward・backwardを行う．`.eval()` はautogradの
        勾配計算グラフの構築を妨げないため，全パラメータに対する `.grad` は通常通り計算
        される．
    引数:
...
def main():
    """
    概要: Ex004（NFG SVRG原論文の実験再現，M_WORKERSワーカーの分散環境模擬版）の
        全条件（4手法 x 5Seed）をマルチプロセスで並列に学習する．GPU（VRAM）の使用量を
        考慮し，並列プロセス数は4とする．
    引数: なし
    戻り値: なし
    """
